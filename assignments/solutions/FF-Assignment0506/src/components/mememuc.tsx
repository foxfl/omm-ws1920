...
import "./mememuc.css";
import History from "./mememuc-history";
...
const OmmMemeMUC: React.FC = () => {
  ...
  const _removeSavedMeme = (meme: CustomizedMeme) => {
    setSavedMemes(savedMemes.filter((me) => me.link !== meme.link));
  };
  const _saveMeme = () => {
    if (url) {
      setSavedMemes([
    ...
    setCaptionState(initalCaptionState);
  };

  const _restoreMeme = (meme: CustomizedMeme) => {
    const { baseMeme, caption } = meme;
    setSelectState(baseMeme);
  ...
  return (
    <>
      ...
      <History
        ...
        onDeleteSavedMeme={_removeSavedMeme}
        onRestoreMeme={_restoreMeme}
      />
    </>
  );
};
